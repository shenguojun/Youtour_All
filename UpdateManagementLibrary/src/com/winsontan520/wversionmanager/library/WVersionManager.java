...
public class WVersionManager implements IWVersionManager {
	...
	private void showDialog() {
		...
		builder.setIcon(getIcon())
				.setTitle(getTitle())
				.setMessage(
						Html.fromHtml(getMessage(), null, getCustomTagHandler()))
	...
	class VersionContentRequest extends AsyncTask<String, Void, String> {
		...
		protected void onPreExecute() {
			...
			mProgressDialog.setTitle("检查更新"); // 设置标题
			mProgressDialog.setMessage("正在检查更新，请耐心等待..."); //
		...
		protected void onPostExecute(String result) {
			...
			if (statusCode != HttpStatus.SC_OK) {
				...
				Toast.makeText(activity, "更新失败，请检查网络", Toast.LENGTH_SHORT).show();
			} else {
				...
				try {
					...
					updateUrl = json.optString("down_url");

					Log.i("remoteVerion", versionCode + "");
					int currentVersionCode = getCurrentVersionCode();
					Log.i("currentVersionCode", currentVersionCode + "");
					if (currentVersionCode < versionCode) {
						// new versionCode will always higher than
						// currentVersionCode
						// if (versionCode != getIgnoreVersionCode()) { // check
						// is
						// new
						// versionCode
						// is
						...
						// show update dialog
						Log.i("startShowDialog", "startShowDialog");

						showDialog();
						// }
					}else{
						Toast.makeText(activity, "已为最新版本，不需更新", Toast.LENGTH_SHORT).show();
					}
